...
public class CaptureCommandImpl extends AbstractCommand implements CaptureCommand
{
	...
	public CaptureResult perform(final CaptureRequest request)
	{
		...
		{
			if (isIntentSale(order) || isAvailableSubmitForSettlementForApplePay(order))
			{
	...
	{
		getLoggingHandler().getLogger().info(String.format("[SETTLEMENT FOR TRANSACTION ID: %s WILL BE SETTLED IMMEDIATELY BY BRAINTREE] See transaction details.", transactionID));
	...
	private boolean isNotPayPalPayment(AbstractOrderModel order)
	{
		...
		return BraintreeConstants.BRAINTREE_PAYMENT.equals(paymentProvider) || BraintreeConstants.APPLE_PAY_PAYMENT.equals(paymentProvider);
	}
	...
	private boolean isApplePay(AbstractOrderModel order) {
				return BraintreeConstants.APPLE_PAY_PAYMENT.equalsIgnoreCase(((BrainTreePaymentInfoModel)order.getPaymentInfo()).getPaymentProvider());
			}

	private boolean isAvailableSubmitForSettlementForApplePay(AbstractOrderModel order)
	{
			return isApplePay(order) && getBrainTreeConfigService().getSettlementConfigParameter();
}
