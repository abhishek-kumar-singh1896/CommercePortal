...
public class PartialCaptureCommandImpl extends AbstractCommand implements BrainTreePartialCaptureCommand {
	...
	public BrainTreeSaleTransactionResult perform(final BrainTreeSaleTransactionRequest saleRequest)
	{
        ...
        try {
            final Result<Transaction> result = getBraintreeGateway().transaction().submitForPartialSettlement(saleRequest.getRequestId(), saleRequest.getTotalAmount());
            if (result.isSuccess()) {
	...
	private BrainTreeSaleTransactionResult translateErrorResponse(final Result<Transaction> result)
	{
        ...
        if (result.getErrors() != null) {
            ...
            if (CollectionUtils.isNotEmpty(allDeepValidationErrors)) {
                ...
                getLoggingHandler().getLogger().info(String.format("BT response error: %s, %s", validationError.getCode(), validationError.getMessage()));
	...
	private void addInVaultProperty(final TransactionOptionsRequest options)
	{
		...
		{
			options.storeInVault(Boolean.valueOf(getBrainTreeConfigService().getStoreInVault()));
		}
