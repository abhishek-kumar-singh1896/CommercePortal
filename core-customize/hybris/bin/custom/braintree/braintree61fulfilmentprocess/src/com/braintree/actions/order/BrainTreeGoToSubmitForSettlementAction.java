...
public class BrainTreeGoToSubmitForSettlementAction extends AbstractSimpleDecisionAction<OrderProcessModel> {
    ...
    private boolean canCreateCaptureTransactionForOrder(OrderModel order)
    {
        ...
        if (BraintreeConstants.PAYPAL_PAYMENT.equals(paymentProvider) || BrainTreePaymentMethod.PAYPAL.getCode().equals(paymentProvider) || BraintreeConstants.PAY_PAL_EXPRESS_CHECKOUT.equals(paymentProvider))
        {
            String intent = ((BrainTreePaymentInfoModel) order.getPaymentInfo()).getPayPalIntent();
            return BraintreeConstants.PAYPAL_INTENT_SALE.equalsIgnoreCase(intent) || BraintreeConstants.PAYPAL_INTENT_ORDER.equalsIgnoreCase(intent);
        }
