													...
													<div class="row">
														<c:forEach items="${searchPageData.results}" var="result">
															<spring:url
																...
																var="viewUserGroupUrl" htmlEscape="false">
															...
															<div
																id="row-${fn:escapeXml(ycommerce:normalizedCode(result.uid))}"
																class="col-xs-12 col-sm-6 col-md-4 card <c:if test='${result.selected}'>selected</c:if>">
																<ul class="pull-left">
																	<li><ycommerce:testId code="${action}_id_link">
																			<a href="${fn:escapeXml(viewUserGroupUrl)}">${fn:escapeXml(result.uid)}</a>
																		</ycommerce:testId></li>
																	<li><ycommerce:testId code="${action}_name_link">${fn:escapeXml(result.name)}</ycommerce:testId>
																	</li>
																...
																<ycommerce:testId code="${action}_actions_label">
																	<span
																		id="selection-${fn:escapeXml(ycommerce:normalizedCode(result.uid))}"
																		class="account-cards-actions"> <spring:url
