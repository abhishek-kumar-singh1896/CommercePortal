			...
			<div class="col-sm-12 col-md-8 right-nav-content">
				<div>
					<org-common:headline url="${backUrl}"
						labelKey="text.company.managePermissions.viewDetails.page.title" />
				</div>
						...
						<div class="row">
									...
									<div class="col-sm-4 item-wrapper">
										<div class="item-group">
											<span class="item-label"> <spring:theme
												...
												${fn:escapeXml(permissionData.code)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.managePermissions.unit.label" />
											</span> <span class="item-value"> <a
												href="${fn:escapeXml(unitDetailsUrl)}">${fn:escapeXml(permissionData.unit.name)}</a>
											</span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.permissions.type.label" />
											</span> <span class="item-value">
												${fn:escapeXml(permissionData.b2BPermissionTypeData.name)} </span>
										</div>
									</div>
									<div class="col-sm-4 item-wrapper">
										<div class="item-group">
											...
											</span> <span class="item-value"> <company:permissionTimespan
										...
										<div class="item-group">
											...
											</span> <span class="item-value"> <company:permissionAmount
													permission="${permissionData}"
													displayBracketsAroundCurrency="true" />
											</span>
										</div>
									</div>
									<div class="col-sm-4 item-wrapper">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.column.status.name" />
											</span> <span class="item-value"> <c:choose>
													<c:when test="${permissionData.active}">
														<spring:theme
															code="text.company.managePermissions.status.enabled" />
													</c:when>
													<c:otherwise>
														<spring:theme
															code="text.company.managePermissions.status.disabled" />
													</c:otherwise>
												</c:choose>
											</span>
										</div>
							...
							<div class="col-sm-2">
								<div class="item-action">
									<a href="${fn:escapeXml(editPermissionDetailsUrl)}"
						...
						<c:choose>
							<c:when test="${permissionData.active}">
								<div class="js-action-confirmation-modal disable-link">
									<a href="#"
										data-action-confirmation-modal-title="<spring:theme code="text.company.permission.disable.confirm"/>"
										data-action-confirmation-modal-id="disable"> <spring:theme
											code="text.company.permission.disable.confirm" />
									</a>
								</div>
								<company:actionConfirmationModal id="disable"
									targetUrl="${confirmDisableUrl}"
