...
<template:page pageTitle="${pageTitle}">
			...
			<div class="col-md-8">
				<div class="row">
					<div class="col-xs-12 col-sm-7">
						<org-common:headline url="${cancelUrl}"
							labelKey="text.company.manageusers.${action}.title" />
					</div>
				...
				<div class="account-section-content">
					<nav:pagination top="true" showTopTotals="false"
						...
						additionalParams="${sortQueryParams}"
								...
								<div class="account-cards card-select">
									<div class="row">
										<c:forEach items="${searchPageData.results}" var="result">
											<div
												id="row-${fn:escapeXml(ycommerce:normalizedCode(result.code))}"
												class="col-xs-12 col-sm-6 col-md-4 card <c:if test='${result.selected}'>selected</c:if>">
												<company:permissionCardDetails permission="${result}"
													action="${action}" listCSSClass="pull-left" />
												<ycommerce:testId code="${action}_actions_label">
													<span
														id="span-${fn:escapeXml(ycommerce:normalizedCode(result.code))}"
														class="account-cards-actions pull-left"> <spring:url
															value="${baseUrl}/{/action}/select/" var="selectUrl"
															htmlEscape="false">
														...
														</spring:url> <spring:url value="${baseUrl}/{/action}/deselect/"
															...
															<c:when test="${result.selected}">
																<a href="#" url="${fn:escapeXml(deselectUrl)}"
																	class="action-links js-deselectLink"> <span
																	class="glyphicon glyphicon-ok"></span>
																</a>
															</c:when>
															<c:otherwise>
																<a href="#" url="${fn:escapeXml(selectUrl)}"
																	class="action-links js-selectLink"> <span
																	class="glyphicon glyphicon-ok"></span>
																</a>
															</c:otherwise>
														</c:choose>
    ...
    <script id="enableDisableLinksTemplate" type="text/x-jquery-tmpl">
        {{if selected}}
        <a href="#" url="${fn:escapeXml(actionLink)}/deselect/?permission={{= encodeURIComponent(id)}}&amp;user=${fn:escapeXml(ycommerce:encodeUrl(param.user))}" class="action-links js-deselectLink"><span class="glyphicon glyphicon-ok"></span></a>
        {{else}}
        <a href="#" url="${fn:escapeXml(actionLink)}/select/?permission={{= encodeURIComponent(id)}}&amp;user=${fn:escapeXml(ycommerce:encodeUrl(param.user))}" class="action-links js-selectLink"><span class="glyphicon glyphicon-ok"></span></a>
        {{/if}}
    </script>
</template:page>