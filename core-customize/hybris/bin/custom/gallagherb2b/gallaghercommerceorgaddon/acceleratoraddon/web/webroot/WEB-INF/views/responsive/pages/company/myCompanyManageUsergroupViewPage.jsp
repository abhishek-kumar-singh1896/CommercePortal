	...
	<div class="account-section content-inner">
		<div class="row">
			...
			<div class="col-sm-12 col-md-8 right-nav-content">
				<div>
					<org-common:headline url="${backToUserGroupsUrl}"
						labelKey="text.company.manageUsergroups.detailsUserGroup.title" />
				</div>
				<div class="account-section-content">
									...
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.usergroup.id.title" />
											</span> <span class="item-value">
												${fn:escapeXml(usergroup.uid)} </span>
										</div>
									</div>
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.usergroup.name.title" />
											</span> <span class="item-value">
												${fn:escapeXml(usergroup.name)} </span>
										</div>
									</div>
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.usergroup.unit.title" />
											</span> <span class="item-value">
												${fn:escapeXml(usergroup.unit.name)} </span>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="item-action">
									<a href="${fn:escapeXml(editUrl)}"
										class="button edit btn btn-block btn-primary"> <spring:theme
											code="text.company.usergroup.edit.button" />
									</a>
								</div>
							</div>
						</div>
					...
					<div class="accountActions-link">
						<c:if test="${usersSelected}">
							<span class="js-action-confirmation-modal disable-link"> <ycommerce:testId
									code="Usergroup_Disable_button">
									<a href="#"
										data-action-confirmation-modal-title="<spring:theme code="text.company.usergroup.disable.label"/>"
										data-action-confirmation-modal-id="disable" class=""> <spring:theme
											code="text.company.usergroup.disable.label" />
									</a>
								</ycommerce:testId>
							</span>
						</c:if>
						<span class="js-action-confirmation-modal remove disable-link">
							<ycommerce:testId code="Usergroup_Remove_button">
								<a href="#"
									data-action-confirmation-modal-title="<spring:theme code="text.company.usergroup.delete.label"/>"
									data-action-confirmation-modal-id="delete"> <spring:theme
										code="text.company.usergroup.delete.label" />
								</a>
							</ycommerce:testId>
						</span>
					</div>

					<company:actionConfirmationModal id="delete"
						targetUrl="${deleteUrl}"
					...
					<div class="account-list">
						...
						<org-common:selectEntityHeadline url="${membersUrl}"
							labelKey="text.company.usergroup.members" />
							...
							<div class="account-cards">
								<div class="row">
									<c:forEach items="${usergroup.members}" var="user">
										...
										<div class="col-xs-12 col-sm-6 col-md-4 card">
											<ul>
												<li><ycommerce:testId code="member_name_link">
														<a href="${fn:escapeXml(userDetailUrl)}">${fn:escapeXml(user.name)}</a>
													</ycommerce:testId></li>
												<li><ycommerce:testId code="member_email_label">
					...
					<div class="row">
						<div class="col-xs-12 col-sm-5 col-md-4">
							<div class="accountActions-bottom">
								<org-common:back cancelUrl="${backToUserGroupsUrl}"
									displayTextMsgKey="text.company.usergroup.back.button" />
							</div>
						</div>
					</div>
				</div>
