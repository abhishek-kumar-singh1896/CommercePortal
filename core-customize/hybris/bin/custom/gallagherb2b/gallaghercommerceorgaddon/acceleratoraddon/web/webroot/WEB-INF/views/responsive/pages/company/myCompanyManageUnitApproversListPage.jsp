	...
	<div class="account-section content-inner">
		<div class="row">
			...
			<div class="col-sm-12 col-md-8 right-nav-content">
				<div class="row">
					<div class="col-xs-12 col-sm-7">
						<org-common:headline url="${cancelUrl}"
							labelKey="text.company.manage.units.${action}.mainTitle" />
					</div>
				...
				<div
					class="account-section-content ${empty searchPageData.results ? 'content-empty' : ''}">
								...
								<div class="account-cards card-select">
									<div class="row">
										<c:forEach items="${searchPageData.results}" var="user">
											...
											<div
												class="col-xs-12 col-sm-6 col-md-4 card ${user.selected ? 'selected' : ''}"
												id="row-${fn:escapeXml(user.normalizedUid)}">
												<ul class="pull-left">
													<li><ycommerce:testId code="name_${action}_link">
															<a href="${fn:escapeXml(viewUrl)}">${fn:escapeXml(user.name)}</a>
														</ycommerce:testId></li>
													<li><ycommerce:testId code="email_${action}_label">
                                                    ${fn:escapeXml(user.uid)}
                                                </ycommerce:testId></li>
													<li><ycommerce:testId code="b2bunit_${action}_label">
													...
													<li class="roles-wrapper clearfix"><b><spring:theme
															...
															code="roles_${action}_label">
															<ul id="roles-${fn:escapeXml(user.normalizedUid)}">
																<c:forEach items="${user.roles}" var="role">
																	<li><spring:theme code="b2busergroup.${role}.name" />
																	</li>
																</c:forEach>
															</ul>
														</ycommerce:testId></li>
													<li><ycommerce:testId code="status_${action}_label">
															<spring:theme
																code="text.company.status.active.${user.active}" />
														</ycommerce:testId></li>
												</ul>
