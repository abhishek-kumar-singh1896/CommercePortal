				...
				<div class="account-section-content">
									...
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.unit.id" />
											</span> <span class="item-value"> ${fn:escapeXml(unit.uid)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.unit.name" />
											</span> <span class="item-value"> ${fn:escapeXml(unit.name)}
											</span>
										</div>
									</div>
										...
										<div class="col-sm-4">
											<div class="item-group">
												<span class="item-label"> <spring:theme
													...
													${fn:escapeXml(unit.unit.uid)} </span>
											</div>
										</div>
									</c:if>
								...
								<c:choose>
									<c:when test="${unit.unit.active}">
										<ycommerce:testId code="Unit_EnableUnit_button">
											<a href="${fn:escapeXml(enableUnitUrl)}"
					...
					<div class="account-list">
						...
						<div class="account-list-header">
							<spring:theme code="text.company.manage.units.addresses" />
							(${fn:length(unit.addresses)})
						</div>
						...
						<div class="account-list-header">
							<spring:theme code="text.company.manage.units.accountManagers" />
							(${fn:length(unit.accountManagers)})
						</div>
						<div class="account-cards">
							<div class="row">
						...
						<org-common:selectEntityHeadline url="${addCostcenterUrl}"
							labelKey="text.company.manage.units.costCenters"
									...
									<div class="col-xs-12 col-sm-6 col-md-4 card">
										<spring:url
											...
											<spring:param name="costCenterCode"
												value="${b2bCostCenter.code}" />
										</spring:url>
										<ul class="pull-left">
											<li><a href="${fn:escapeXml(viewCostcenterUrl)}">${fn:escapeXml(b2bCostCenter.code)}</a>
											</li>
							...
							<div class="row">
								<c:forEach items="${unit.children}" var="unit">
									<div class="col-xs-12 col-sm-6 col-md-4 card">
										...
										<ul class="pull-left">
											<li><a href="${fn:escapeXml(unitUrl)}">${fn:escapeXml(unit.uid)}</a>
											</li>
						...
						<spring:url
							value="/my-company/organization-management/manage-units/technicians"
							var="editTechniciansUrl" htmlEscape="false">
							<spring:param name="unit" value="${unit.uid}" />
							<spring:param name="role" value="b2btechniciangroup" />
						</spring:url>


						<company:userList users="${unit.technicians}" action="technicians"
							role="b2btechniciangroup" editUrl="${editTechniciansUrl}"
