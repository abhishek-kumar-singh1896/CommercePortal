	...
	<div class="account-section content-inner">
		<div class="row">
			...
			<div class="col-sm-12 col-md-8 right-nav-content">
				<div>
					<org-common:headline url="${backUrl}"
						labelKey="text.company.budget.viewPage.label" />
				</div>
						...
						<div class="row">
							<div class="col-sm-10 col-no-padding">
								<div class="row">
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.budgetId" />
											</span> <span class="item-value">
												${fn:escapeXml(budget.code)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.name" />
											</span> <span class="item-value">
												${fn:escapeXml(budget.name)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.unit" />
											</span> <span class="item-value"> <a
												href="${fn:escapeXml(viewBudgetUnitUrl)}">${fn:escapeXml(budget.unit.name)}</a>
											</span>
										</div>
									</div>
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.startDate" />
											</span> <span class="item-value">
												${fn:escapeXml(budget.startDate)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.endDate" />
											</span> <span class="item-value">
												${fn:escapeXml(budget.endDate)} </span>
										</div>
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.amount" />
											</span> <span class="item-value"> <fmt:formatNumber
													value="${budget.budget}" maxFractionDigits="2"
													minFractionDigits="2" />
									...
									<div class="col-sm-4">
										<div class="item-group">
											<span class="item-label"> <spring:theme
													code="text.company.budget.enabledDisabled" />
							...
							<div class="col-sm-2">
								<div class="item-action">
									<a href="${fn:escapeXml(editBudgetDetailsUrl)}"
										id="EditBudget_button"
						...
						<c:choose>
							<c:when test="${budget.active}">
								<div class="js-action-confirmation-modal disable-link">
									<a href="#"
										data-action-confirmation-modal-title="<spring:theme code="text.company.budget.disable.confirm"/>"
								...
								<c:choose>
									<c:when test="${not empty budget.costCenters}">
										<c:forEach items="${budget.costCenters}" var="costCenter">
											...
											<div class="col-xs-12 col-sm-6 col-md-4 card">
												<ul>
													<li><a href="${fn:escapeXml(viewCostCenterUrl)}">${fn:escapeXml(costCenter.code)}</a>
													</li>
													<li>
									...
									<c:otherwise>
										<div class="search-empty">
											<spring:theme
												code="text.company.budget.noCostCentersFound.message" />
										</div>
									</c:otherwise>
								</c:choose>
							</div>
					...
					<div class="row">
						<div class="col-xs-12 col-sm-5 col-md-4">
							<div class="accountActions-bottom">
								<org-common:back cancelUrl="${backUrl}"
									displayTextMsgKey="text.company.budget.viewPage.back" />
							</div>
						</div>
					</div>
				</div>
