			...
			<div class="col-md-8">
				<div class="account-section-header">
					...
					<div class="account-section-header-add pull-right">
						<a href="${fn:escapeXml(addBudgetUrl)}" class="button add"><spring:theme
								code="text.company.budget.createButton.displayName" /></a>
					</div>
				</div>
						...
						<div class="account-overview-table">
							<table class="responsive-table">
								<tr class="responsive-table-head hidden-xs">
									...
									<th><spring:theme
											code="text.company.manage.units.costCenters" /></th>
								...
								<c:forEach items="${searchPageData.results}" var="b2bBudget">
									...
									<tr class="responsive-table-item">
										...
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_id_link">
												<a href="${fn:escapeXml(viewBudgetDetailsUrl)}"
													class="responsive-table-link">${fn:escapeXml(b2bBudget.code)}</a>
											</ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.column.name.name" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_name_link">
                                        ${fn:escapeXml(b2bBudget.name)}
                                    </ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.column.parentUnit.name" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_unit_label">
												<a href="${fn:escapeXml(unitDetailsUrl)}"
													class="responsive-table-link">${fn:escapeXml(b2bBudget.unit.name)}</a>
											</ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.manage.units.costCenters" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_unit_label">
												<c:forEach items="${b2bBudget.costCenterNames}"
													var="costCenter">
                                            ${fn:escapeXml(costCenter)}<br />
												</c:forEach>
											</ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.budget.amount" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_amount_label">
												<fmt:formatNumber value="${b2bBudget.budget}"
													maxFractionDigits="2" minFractionDigits="2" /> (${fn:escapeXml(b2bBudget.currency.isocode)})
                                    </ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.budget.start" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_startDate_label">
												<fmt:formatDate value="${b2bBudget.startDate}" />
											</ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.budget.end" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_endDate_label">
												<fmt:formatDate value="${b2bBudget.endDate}" />
											</ycommerce:testId></td>
										<td class="hidden-sm hidden-md hidden-lg"><spring:theme
												code="text.company.column.status.name" /></td>
										<td class="responsive-table-cell"><ycommerce:testId
												code="budget_status_label">
												<c:choose>
													<c:when test="${b2bBudget.active}">
														<span><spring:theme
																code="text.company.budget.status.active" /></span>
													</c:when>
													<c:otherwise>
														<span class="account-status-inactive"><spring:theme
																code="text.company.budget.status.disabled" /></span>
													</c:otherwise>
												</c:choose>
											</ycommerce:testId></td>
									</tr>
