 ...
 */
package com.gallagher.gallaghercustomerticketaddon.controllers.pages;
...
import com.gallagher.gallaghercustomerticketaddon.constants.GallaghercustomerticketaddonConstants;
import com.gallagher.gallaghercustomerticketaddon.forms.SupportTicketForm;
...
public class GallagherAccountSupportTicketsPageController extends AbstractSearchPageController
{
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.SUPPORT_TICKETS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.SUPPORT_TICKETS_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY));
		...
		if (ticketAdded)
		{
			GlobalMessages.addConfMessage(model, GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ADDED);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.ADD_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.ADD_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ADD));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(GallaghercustomerticketaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		...
		try
		{
			...
			model.addAttribute(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_CATEGORIES, ticketFacade.getTicketCategories());
		}
			...
			final BindingResult bindingResult)
	{
		...
		if (bindingResult.hasErrors())
		{
			...
			list.add(buildMessageMap(GallaghercustomerticketaddonConstants.FORM_GLOBAL_ERROR_KEY,
					GallaghercustomerticketaddonConstants.FORM_GLOBAL_ERROR));
		...
		catch (final UnsupportedAttachmentException usAttEx)
		{
			...
			map.put(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource()
					.getMessage(GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ATTACHMENT_BLOCK_LISTED, new Object[]
		...
		catch (final RuntimeException rEX)
		{
			...
			map.put(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource().getMessage(
					GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null, getI18nService().getCurrentLocale()));
			return new ResponseEntity<>(Lists.newArrayList(map), HttpStatus.BAD_REQUEST);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(GallaghercustomerticketaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_UPDATE));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(GallaghercustomerticketaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(GallaghercustomerticketaddonConstants.MAX_UPLOAD_SIZE_MB,
		...
		try
		{
			...
			model.addAttribute(GallaghercustomerticketaddonConstants.SUPPORT_TICKET_DATA, ticketData);
		}
		catch (final Exception e)
		{
			LOG.error("Attempted to load ticket details that does not exist or is not visible", e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null);
	...
	protected List<Breadcrumb> getBreadcrumbs(final String breadcrumbCode)
	{
		...
		breadcrumbs.add(new Breadcrumb("/my-account/support-tickets", getMessageSource().getMessage(
				GallaghercustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY, null, getI18nService().getCurrentLocale()), null));
		breadcrumbs.add(
