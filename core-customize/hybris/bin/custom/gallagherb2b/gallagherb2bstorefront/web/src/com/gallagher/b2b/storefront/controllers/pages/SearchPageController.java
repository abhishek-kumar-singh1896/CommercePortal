...
import com.gallagher.b2b.storefront.util.CompareUtil;
...
public class SearchPageController extends AbstractSearchPageController
{
	...
	final String searchText, final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		...
		final String mindtouchSRC = getConfiguration().getString("mindtouch.src");
		final String mindtouchID = getConfiguration().getString("mindtouch.id");
		...
		final List<ProductData> productComparisonList = productComparisonFacade.getProductComparisonList();
		model.addAttribute("comparedProducts", productComparisonList);
		if (!org.fest.util.Collections.isEmpty(productComparisonList))
		{
