...
<spring:url value="/my-account/quick-order-templates/" var="orderTemplatesLink" htmlEscape="false"/>
<c:set var="searchUrl" value="/my-account/quick-order-templates?sort=${ycommerce:encodeUrl(searchPageData.pagination.sortCode)}"/>

<div class="account-section-header">
    <spring:theme code="text.account.quickOrderTemplates"/>
</div>
<c:if test="${empty searchPageData.results}">
    <div class="account-section-content content-empty">
        <ycommerce:testId code="savedCarts_noOrders_label">
            <spring:theme code="text.account.quickOrderTemplates.noOrderTemplates"/>
        </ycommerce:testId>
    </div>
</c:if>
...
<c:if test="${not empty searchPageData.results}">
    <div class="account-section-content">
        <div class="account-orderhistory-pagination">
            <nav:advancepagination top="true" msgKey="text.account.quickOrderTemplates.page" showCurrentPageInfo="true"
                    ...
                    <tr id="row-${loop.index}" class="responsive-table-item"> 		
						...
						<td class="responsive-table-cell">
						 ...
						 <td class="responsive-table-cell restore-item-column text-center">
							<ycommerce:testId code="savedCarts_restore_link">
								<a href="#" class="js-restore-order-template restore-item-link ${importCartIsProcessing || fn:length(savedCart.entries) < 1 ? 'hidden' : '' }"
									data-savedcart-id="${fn:escapeXml(savedCart.code)}"
									data-restore-popup-title="<spring:theme code='text.account.quickOrderTemplates.restore.popuptitle'/>">
						...
						<td class="responsive-table-cell remove-item-column">
							<ycommerce:testId code="savedCarts_delete_link">
								<a href="#" class="js-delete-order-template remove-item-link ${importCartIsProcessing ? 'hidden' : '' }"
									data-savedcart-id="${fn:escapeXml(savedCart.code)}"
									data-delete-popup-title="<spring:theme code='text.account.ordertemplate.delete.popuptitle'/>">
									<span class="glyphicon glyphicon-remove"></span>
								</a>
							</ycommerce:testId>
						</td>
                		<cart:templateDeleteModal savedCart="${savedCart}"/>                 
                    </tr>
                </c:forEach>
            </table>
        ...
        <div class="account-orderhistory-pagination">
            <nav:advancepagination top="false" msgKey="text.account.quickOrderTemplates.page" showCurrentPageInfo="true"
                            hideRefineButton="true"
                            supportShowPaged="${isShowPageAllowed}" supportShowAll="${isShowAllAllowed}"
                            searchPageData="${searchPageData}" searchUrl="${searchUrl}"
                            numberPagesShown="${numberPagesShown}"/>
