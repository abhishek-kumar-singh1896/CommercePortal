...
ACC.checkoutCustom = {
	...
	{
		ACC.checkoutCustom.toggleActionButtons('.place-order-form');
	...
	toggleActionButtons: function(selector) {
		...
		if(checkBox.is(':checked')) {
            ...
            ACC.checkoutCustom.disableQuoteButton(cssClass);
		}
	...
	bindScheduleReplenishment: function(data)
	{
		...
		$(document).on("click",".scheduleReplenishmentButton",function(e){
			...
			ACC.colorbox.open(title,{
				...
				width:"620px",
				onComplete: function(){
					ACC.checkoutCustom.toggleReplenishmentScheduleDateError(false);
		...
		$(document).on("click",'#replenishmentSchedule #placeReplenishmentOrder', function (e){
			...
			if(ACC.checkoutCustom.validateDate(dateEntered, localeDateFormat)){
				$(".replenishmentOrderClass").val(true);
				placeReplenishment = true;
				$.colorbox.close();	
			} else{
				ACC.checkoutCustom.toggleReplenishmentScheduleDateError(true);
				$.colorbox.resize();
			}
