 ...
 */
package com.gallager.customerticketingaddon.controllers.pages;
...
import com.gallager.customerticketingaddon.constants.GallaghercustomerticketingaddonConstants;
import com.gallager.customerticketingaddon.controllers.forms.SupportTicketForm;
...
public class AccountSupportTicketsPageController extends AbstractSearchPageController
{
	...
	@RequireHardLogIn
	public String supportTickets(@RequestParam(value = "page", defaultValue = "0")
	final int pageNumber, @RequestParam(value = "show", defaultValue = "Page")
	final ShowMode showMode, @RequestParam(value = "sort", required = false)
	final String sortCode, @RequestParam(value = "ticketAdded", required = false, defaultValue = "false")
	final boolean ticketAdded, final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.SUPPORT_TICKETS_PAGE));
		setUpMetaDataForContentPage(model,
				getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.SUPPORT_TICKETS_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY));
		...
		if (ticketAdded)
		{
			GlobalMessages.addConfMessage(model, GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_ADDED);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.ADD_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model,
				getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.ADD_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_ADD));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(GallaghercustomerticketingaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(GallaghercustomerticketingaddonConstants.MAX_UPLOAD_SIZE_MB,
				FileUtils.byteCountToDisplaySize(maxUploadSizeValue));
		try
		{
			model.addAttribute(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_ASSOCIATED_OBJECTS,
					ticketFacade.getAssociatedToObjects());
			model.addAttribute(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_CATEGORIES,
			...
			final BindingResult bindingResult)
	{
		...
		if (bindingResult.hasErrors())
		{
			...
			list.add(buildMessageMap(GallaghercustomerticketingaddonConstants.FORM_GLOBAL_ERROR_KEY,
					GallaghercustomerticketingaddonConstants.FORM_GLOBAL_ERROR));
		...
		catch (final UnsupportedAttachmentException usAttEx)
		{
			...
			map.put(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource()
					.getMessage(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_ATTACHMENT_BLOCK_LISTED, new Object[]
		...
		catch (final RuntimeException rEX)
		{
			...
			map.put(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_TRY_LATER,
					getMessageSource().getMessage(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null,
	...
	@RequireHardLogIn
	public String getSupportTicket(@PathVariable("ticketId")
	final String ticketId, final Model model, @RequestParam(value = "ticketUpdated", required = false, defaultValue = "false")
	final boolean ticketUpdated, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model,
				getContentPageForLabelOrId(GallaghercustomerticketingaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_UPDATE));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(GallaghercustomerticketingaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(GallaghercustomerticketingaddonConstants.MAX_UPLOAD_SIZE_MB,
		...
		try
		{
			...
			model.addAttribute(GallaghercustomerticketingaddonConstants.SUPPORT_TICKET_DATA, ticketData);
		}
		catch (final Exception e)
		{
			LOG.error("Attempted to load ticket details that does not exist or is not visible", e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null);
	...
	protected List<Breadcrumb> getBreadcrumbs(final String breadcrumbCode)
	{
		...
		breadcrumbs.add(new Breadcrumb("/my-account/support-tickets",
				getMessageSource().getMessage(GallaghercustomerticketingaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY, null,
						getI18nService().getCurrentLocale()),
				null));
		breadcrumbs.add(
