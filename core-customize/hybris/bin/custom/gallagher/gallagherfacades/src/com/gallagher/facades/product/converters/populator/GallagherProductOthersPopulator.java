 ...
 */
public class GallagherProductOthersPopulator<SOURCE extends ProductModel, TARGET extends ProductData>
		extends AbstractProductImagePopulator<SOURCE, TARGET>
{
	...
	public void populate(final SOURCE productModel, final TARGET productData) throws ConversionException
	{
		...
		if (logos != null)
		{
			...
			addLogos(logos, ImageDataType.OTHERS, imageList);
			...
			productData.setOthers(imageList);
		}
	}

	protected Collection<MediaModel> getPrimaryImageMediaContainer(final SOURCE productModel)
	{
		final Collection<MediaModel> logos = (Collection<MediaModel>) getProductAttribute(productModel, ProductModel.OTHERS);
		//this is else part
	...
	protected void addLogos(final Collection<MediaModel> logos, final ImageDataType imageType, final List<ImageData> list)
	{
		for (final MediaModel logo : logos)
		{
			...
			{
				imageData.setFormat(logo.getMediaFormat().getQualifier());
			}
