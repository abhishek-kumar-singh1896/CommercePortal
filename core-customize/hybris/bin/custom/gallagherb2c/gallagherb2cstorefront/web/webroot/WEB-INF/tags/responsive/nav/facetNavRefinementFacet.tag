...
<!-- 											</label> -->
											<div class="custom-control custom-checkbox mb-2">
												<input type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''} class="custom-control-input"
													id="${(facetData.code).concat(status.index).concat('Desktop')}">
												<label class="custom-control-label" for="${(facetData.code).concat(status.index).concat('Desktop')}">
													${fn:escapeXml(facetValue.name)}
												</label>
											</div>
										</form>
				...
				<c:if test="${not empty facetData.topValues}">
					<span class="facet__values__more js-more-facet-values">
						<a href="#" class="js-more-facet-values-link" >
							<spring:theme code="search.nav.facetShowMore_${facetData.code}" />
						</a>
					</span>
					<span class="facet__values__less js-less-facet-values">
						<a href="#" class="js-less-facet-values-link">
							<spring:theme code="search.nav.facetShowLess_${facetData.code}" />
						</a>
					</span>
				</c:if>
				</div>
