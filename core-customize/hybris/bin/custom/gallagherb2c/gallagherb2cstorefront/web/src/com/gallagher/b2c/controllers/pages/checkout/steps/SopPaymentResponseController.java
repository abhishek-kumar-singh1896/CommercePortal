...
import com.gallagher.b2c.web.payment.validation.GallagherSopPaymentDetailsValidator;
...
public class SopPaymentResponseController extends PaymentMethodCheckoutStepController
{
	...
	@Resource(name = "gallagherSOPPaymentDetailsValidator")
	private GallagherSopPaymentDetailsValidator sopPaymentDetailsValidator;

	@RequestMapping(value = "/response", method = RequestMethod.POST)
	@RequireHardLogIn
	public String doHandleSopResponse(final HttpServletRequest request, @Valid
	final SopPaymentDetailsForm sopPaymentDetailsForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		sopPaymentDetailsValidator.validate(sopPaymentDetailsForm, bindingResult);
		...
		if (bindingResult.hasErrors())
		{
			...
			((GallagherPaymentFacade) (this.getPaymentFacade())).parseMissingFields(resultMap, errors);
			/* Remove the errors generated by the current address validation */
			for (final FieldError fieldError : bindingResult.getFieldErrors())
			{
				errors.remove(fieldError.getField());
			}
			paymentSubscriptionResultData = new PaymentSubscriptionResultData();
			paymentSubscriptionResultData.setErrors(errors);
		}
		else
		{
			paymentSubscriptionResultData = this.getPaymentFacade().completeSopCreateSubscription(resultMap, savePaymentInfo, false);
		}
		if (paymentSubscriptionResultData.isSuccess())
		{
			createNewPaymentSubscription(paymentSubscriptionResultData);
				...
				|| (MapUtils.isNotEmpty(paymentSubscriptionResultData.getErrors())) || bindingResult.hasErrors())
		{
			...
			throws CMSItemNotFoundException
	{
		...
		model.addAttribute("checkUseDelievryMethod", sopPaymentDetailsForm.isUseDeliveryAddress());
			...
			final PaymentSubscriptionResultData paymentSubscriptionResultData)
	{
		...
		if (bindingResult.hasErrors() && !errorAdded)
		{
			GlobalMessages.addErrorMessage(model, "checkout.error.paymentethod.formentry.invalid");
		}
	}
