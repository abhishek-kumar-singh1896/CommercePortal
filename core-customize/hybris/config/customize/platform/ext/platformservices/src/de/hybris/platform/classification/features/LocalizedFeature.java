...
public class LocalizedFeature extends Feature
{
	...
	public List<FeatureValue> getValues(final Locale locale)
	{
		...
		List<FeatureValue> featureValues = values.get(locale);
		if (featureValues == null)
		{
			featureValues = values.get(Locale.ENGLISH);
			if (featureValues == null)
			{
	...
	public void addValue(final int index, final FeatureValue fvalue, final Locale locale)
	{
		...
		if (featureValues == null)
		{
			...
			Preconditions.checkArgument(index <= _featureValues.size() && index >= 0,
					"index is not in range of: 0 and " + _featureValues.size());
			_featureValues.add(index, fvalue);
			values.put(locale, _featureValues);
		}
		else
		{
			Preconditions.checkArgument(index <= featureValues.size() && index >= 0,
					"index is not in range of: 0 and " + featureValues.size());
